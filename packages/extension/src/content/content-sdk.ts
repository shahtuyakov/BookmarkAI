...
console.log('BookmarkAI Web Clip content script (SDK) loaded');
...
class FloatingActionButton {
  ...
  public async checkAuthStatus() {
    try {
      // Still use messaging for auth status since SDK client is in service worker
  ...
  private async bookmarkPage() {
    ...
    try {
      ...
      // SDK is in service worker, so we still need to send message
      ...
      this.logError('BOOKMARK_ERROR', error);
  ...
  private async logError(type: string, error: any) {
    try {
      ...
      console.error('BookmarkAI: Failed to log error:', e);
    }
  }
