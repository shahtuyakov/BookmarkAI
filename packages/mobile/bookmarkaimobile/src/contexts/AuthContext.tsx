...
async function verifyTokenSync(): Promise<void> {
  ...
  try {
    ...
    if (isValid) {
      ...
      if (Platform.OS === 'android') {
        const resyncTokens = await getTokens();
        if (resyncTokens) {
          const currentTime = Math.floor(Date.now() / 1000);
          const expiresIn = Math.max(0, resyncTokens.expiresAt - currentTime);
          await androidTokenSync.syncTokens(resyncTokens.accessToken, resyncTokens.refreshToken, expiresIn);
        }
      }
    }
  } catch (error) {
...
export const AuthProvider: React.FC<{children: React.ReactNode}> = ({ children }) => {
  ...
  const checkBiometrics = async () => {
    const { available, biometryType: detectedBiometryType } = await biometricService.checkBiometricAvailability();
  ...
  useEffect(() => {
    ...
    const handleAuthError = async () => {
      console.log('🚨 AuthContext: Handling auth error - clearing tokens and redirecting to login');
      setUser(null);
      setError('Authentication failed. Please login again.');
      setIsLoading(false); // Ensure loading is false so navigation can work
      // Clear both React Native and Android tokens on auth error
      await clearTokens();
      await clearAndroidTokens();
    };
