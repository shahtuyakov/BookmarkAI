...
import { getTokens, clearTokens } from '../services/api/client';
import { enhancedTokenSync, EnhancedTokenSyncDebug } from '../services/enhanced-token-sync';

/**
 * Test suite for verifying token synchronization between React Native and native storage
 * Supports both Android (with native token sync) and iOS (with separate token management)
 */
...
export class TokenSyncTestSuite {
  ...
  /**
   * Test 1: Verify React Native tokens exist and are synced to native storage
   */
  static async testTokenSyncFromReactNative(): Promise<void> {
    ...
    try {
      ...
      // Platform-specific native token sync checks
      if (Platform.OS === 'android') {
        await this.testAndroidTokenSync(rnTokens);
      } else if (Platform.OS === 'ios') {
        await this.testIOSTokenSync(rnTokens);
      } else {
        console.log('Token sync tests not available for this platform');
      }
  ...
  /**
   * Test Android native token synchronization
   */
  private static async testAndroidTokenSync(rnTokens: any): Promise<void> {
    console.log('\nStep 2: Checking Android native authentication status...');
    
    if (!ShareHandler?.isAuthenticated) {
      console.log('Android ShareHandler.isAuthenticated not available');
      return;
    }
  ...
  /**
   * Test iOS native token synchronization
   */
  private static async testIOSTokenSync(_rnTokens: any): Promise<void> {
    console.log('\nStep 2: iOS token sync check...');
    console.log('iOS ShareHandler does not have token sync methods');
    console.log('iOS uses different authentication mechanisms (Keychain, etc.)');
    console.log('React Native tokens are managed separately from iOS Share Extension');
    console.log('TOKEN SYNC SKIPPED: Not applicable for iOS platform');
  }
  ...
  static async testManualTokenSync(): Promise<void> {
    ...
    try {
      ...
      if (Platform.OS === 'android') {
        await this.testAndroidManualSync(rnTokens);
      } else if (Platform.OS === 'ios') {
        console.log('Manual token sync not available on iOS');
        console.log('iOS uses different authentication mechanisms');
      } else {
        console.log('Manual token sync not available for this platform');
      }
  ...
  /**
   * Test Android manual token synchronization
   */
  private static async testAndroidManualSync(rnTokens: any): Promise<void> {
    if (!ShareHandler?.clearAuthTokens || !ShareHandler?.syncAuthTokens) {
      console.log('Android ShareHandler sync methods not available');
      return;
    }
  ...
  static async testTokenPersistence(): Promise<void> {
    ...
    console.log('This test checks if tokens persist in native storage');
    console.log('Instructions: Close the app completely and reopen to test persistence');
    
    try {
      if (Platform.OS === 'android') {
        await this.testAndroidTokenPersistence();
      } else if (Platform.OS === 'ios') {
        await this.testIOSTokenPersistence();
      } else {
        console.log('Token persistence test not available for this platform');
      }
  ...
  /**
   * Test Android token persistence
   */
  private static async testAndroidTokenPersistence(): Promise<void> {
    if (!ShareHandler?.isAuthenticated || !ShareHandler?.getTokenDebugInfo) {
      console.log('Android ShareHandler methods not available');
      return;
    }
  ...
  /**
   * Test iOS token persistence
   */
  private static async testIOSTokenPersistence(): Promise<void> {
    console.log('iOS token persistence test:');
    console.log('iOS uses Keychain for secure token storage');
    console.log('React Native tokens are stored separately from iOS Share Extension');
    
    // Check React Native token persistence
    const rnTokens = await getTokens();
    if (rnTokens) {
      console.log('PERSISTENCE SUCCESS: React Native tokens are persisted!');
      console.log('iOS Share Extension uses separate authentication mechanisms');
    } else {
      console.log('PERSISTENCE INFO: No React Native tokens found');
    }
  }
