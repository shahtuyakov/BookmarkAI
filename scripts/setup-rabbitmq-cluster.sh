...
echo "Setting up RabbitMQ cluster from host..."

# Check if nodes are running
if ! docker ps | grep -q rabbitmq-node1; then
    echo "RabbitMQ nodes are not running. Please start them first with:"
    echo "./scripts/start-rabbitmq-cluster.sh"
    exit 1
fi
